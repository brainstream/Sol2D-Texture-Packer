...
#include <Sol2dTexturePackerGui/TabWidget.h>
#include <QMouseEvent>
#include <QTabBar>
...
void TabWidget::mousePressEvent(QMouseEvent * _event)
{
    if(_event->button() == Qt::MiddleButton)
    {
        int tab_index = tabBar()->tabAt(_event->pos());
        if(tab_index != -1)
            emit tabCloseRequested(tab_index);
    }
    QTabWidget::mousePressEvent(_event);
}
